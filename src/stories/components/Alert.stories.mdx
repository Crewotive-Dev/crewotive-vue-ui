...
<Meta title="Components/Alert" component={ CreAlert }
  parameters={{
    docs:{
      description: {
        component: "Use Crewotive’s Alert component for actions in forms, dialogs, and more with support for multiple sizes, states, and more."
      },
    }
  }}
  decorators={[() => ({ template: '<div class="mt-2"><story/></div>' })]}
  argTypes={{
    variant: { description: "Color of alert", control: { type : 'select', options : themeColors.filter(val=>val.match(/-\d$/g)==null) } },
